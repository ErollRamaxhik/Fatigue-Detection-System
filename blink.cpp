...
void getEyesFromFace2(Mat & matface, Mat& matEye, CascadeClassifier eyeCascade)
{
	...
	matEye = matFace(eyes);
	rectangle(matface, matEye, CV_RGB(255, 0, 0), 2, 8, 0);
}

void eyeTracking2(Mat &matCurrentFace, Mat &matPreviousFace , CascadeClassifier eyeCascade) 
{

                 Mat matEyePrevious;
                 Mat matEyeCurrent;

                 getEyesFromFace2(matPreviousFace, matEyePrevious, eyeCascade);
                 getEyesFromFace2(matCurrentFace, matEyeCurrent, eyeCascade);
             ...
             Rect findEye(Mat capturedFace, CascadeClassifier eyeCascade)
             {
                 ...
                 vector<Rect> eyes;
                 eyeCascade.detectMultiScale(capturedFace, eyes, 1.1, 2, 0 | CV_HAAR_SCALE_IMAGE | CV_HAAR_FIND_BIGGEST_OBJECT, cv::Size(30, 30), cv::Size(60, 60));
